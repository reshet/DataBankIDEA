...
//		});
		...
		upm.setValidExtensions("sav","zip");
...
//		});
	...
	private void processUploadResponse(String response)
	{
		try
		{
			if(response.contains("<ZipUploaded>"))
			{
				int start = response.indexOf("<ZipUploaded>")+13;
				int end = response.indexOf("</ZipUploaded>");
				String uploaded = response.substring(start,end);
				Window.alert("Отправлено "+uploaded+" массивов в обработку.");
	...
	private void parseSPSScmd()
	{
		new RPCCall<Void>() {
			...
			public void onSuccess(Void result) {
				Window.alert("Исследование поставлено в очередь обработки.");
			}
