...
public class SimpleZaconList extends TreeItem implements ICatalogizationConcept{
	private final AdminArticleServiceAsync articleService = GWT
    .create(AdminArticleService.class);
	public SimpleZaconList()
	{
		super();
		setText("Все законы");
	...
	public void refreshContents() {
		new RPCCall<ArrayList<ZaconDTO_Light>>() {
			...
			public void onSuccess(ArrayList<ZaconDTO_Light> result) {
				SimpleZaconList.this.removeItems();
				for(ZaconDTO_Light dto:result)
				{
					...
					SimpleZaconList.this.addItem(new ZaconDescItem(dto));
			...
			@Override
			protected void callService(AsyncCallback<ArrayList<ZaconDTO_Light>> cb) {
				articleService.getZaconsAll(cb);
			}
		}.retry(3);
//		articleService.
