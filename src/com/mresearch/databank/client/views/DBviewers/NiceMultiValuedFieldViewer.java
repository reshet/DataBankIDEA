...
public class NiceMultiValuedFieldViewer extends Composite {
	...
	interface MultiValuedFieldUiBinder extends
			UiBinder<Widget, NiceMultiValuedFieldViewer> {
	}

	public NiceMultiValuedFieldViewer() {
	...
	public NiceMultiValuedFieldViewer(MetaUnitMultivaluedDTO dto,HashMap<String,String> fill, String base_name) {
		initWidget(uiBinder.createAndBindUi(this));
		this.dto = dto;
		this.filling = fill;
	...
	private void renderSubUnits()
	{
		...
		for(MetaUnitDTO dto:base)
		{
			if(dto instanceof MetaUnitMultivaluedStructureDTO)
			{
				...
				subunits_table.setWidget(i++, 0, new NiceMultiValuedFieldViewer(dto_str,filling,base_name));
			}else
