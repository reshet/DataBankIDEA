...
public class AdminLawFilesEditView extends Composite{
	...
	interface AdminResearchFilesEditViewUiBinder extends
			UiBinder<Widget, AdminLawFilesEditView> {
	}
	...
	public AdminLawFilesEditView(long research_id,String category,SocioResearchFilesDTO dto) {
	...
	private void doAddFile(final long key,final String desc)
	{
		new RPCCall<Boolean>() {
			...
			protected void callService(AsyncCallback<Boolean> cb) {
				adminLawService.addFileToAccessor(research_id, key, desc,category, cb);
			}
		}.retry(2);
	...
	private void doFetchFiles()
	{
		new RPCCall<SocioResearchFilesDTO>() {
			...
			protected void callService(AsyncCallback<SocioResearchFilesDTO> cb) {
				adminLawService.getFilesInCategory(research_id, category, cb);
			}
		}.retry(2);
	}
	private void doDeleteFile(final long file_id)
	{
		new RPCCall<Boolean>() {
			...
			protected void callService(
				...
				adminLawService.deleteFileFromAccessor(research_id, file_id, cb);
			}
		}.retry(1);
	}
}
