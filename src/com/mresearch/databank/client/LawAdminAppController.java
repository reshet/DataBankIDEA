...
public class LawAdminAppController implements ValueChangeHandler<String>, AppController {
 ...
 // private final StartPageServiceAsync startpageService;
  ...
  private static LawAdminAppControllerUiBinder uiBinder = GWT
			.create(LawAdminAppControllerUiBinder.class);

	interface LawAdminAppControllerUiBinder extends
  ...
  public LawAdminAppController(SimpleEventBus eventBus) {
	//initWidget();
...
//    });
  ...
  private void doViewLawbank() {
    History.newItem("lawadmin-databank");
  }
  private void doViewArticlesCatalog() {
	    History.newItem("lawadmin-articles");
  }
  ...
  public void onValueChange(ValueChangeEvent<String> event) {
    ...
    if (token != null) {
      ...
      if(token.equals("lawadmin-databank")){
    	  presenter = new AdminLawPerspectivePresenter(articleService, eventBus,new AdminLawPerspectiveView(eventBus));
