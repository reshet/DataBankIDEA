...
import com.mresearch.databank.client.views.PublicationDescItem;
import com.mresearch.databank.client.views.PublicationDetailedView;
...
public class AdminPubPerspectivePresenter implements Presenter
{
	 ...
	 public AdminPubPerspectivePresenter(AdminArticleServiceAsync rpcArticleService, SimpleEventBus eventBus,
		      Display view) {
	...
	public void bind()
	{
		display.getTree().addMouseDownHandler(new MouseDownHandler() {
			...
			public void onMouseDown(MouseDownEvent event) {
				...
				if (it instanceof SimplePublicationList)
				{
					//fetchResearchListData();
					eventBus.fireEvent(new AddArticleEnabledEvent());
				}else if (it instanceof PublicationDescItem)
				{
					final PublicationDescItem rv = (PublicationDescItem)it;
					new RPCCall<PublicationDTO>() {

						@Override
						public void onFailure(Throwable caught) {
							Window.alert("Error on fetching Publication det");
						}
						@Override
						public void onSuccess(final PublicationDTO result) {
						...
						protected void callService(AsyncCallback<PublicationDTO> cb) {
							rpcArticleService.getPublication(rv.getContents_id(), cb);
						}
					}.retry(2);
...
//					ArrayList<PublicationDTO> dtos = new ArrayList<PublicationDTO>();
//					
...
//						@Override
//						protected void callService(
//								AsyncCallback<ArrayList<PublicationDTO>> cb) {
//							rpcArticleService.getPublicationsAll(cb);
//						}
//					}.retry(2);
				}
...
//				if (!(it instanceof SimplePublicationList))
//				{
//					eventBus.fireEvent(new AddPublicationDisabledEvent());
//				}
			}
		});
		...
		display.getTreeForOpen().addOpenHandler(new OpenHandler<TreeItem>() {
			...
			public void onOpen(OpenEvent<TreeItem> event) {
				...
				{
//					fetchResearchListData();
					((SimplePublicationList)it).refreshContents();
					eventBus.fireEvent(new AddArticleEnabledEvent());
...
//					fetchResearchVarData(it, rv.getResearch_id());
				...
				}else if (it instanceof ConceptItem<?>)
				{
					try{
						...
						ConceptItem<PublicationDTO> ct = (ConceptItem<PublicationDTO>)it;
						if (!ct.isBinderAttached())
						{
							ct.attachCBinder(new ConceptBinder<PublicationDTO>(){
								@Override
								public void loadContents(
									...
									new RPCCall<ArrayList<PublicationDTO>>() {
										...
										@Override
										public void onSuccess(ArrayList<PublicationDTO> result) {
											ArrayList<PublicationDTO> handler = getDtos_handler();
											handler.clear();
											for(PublicationDTO dto:result)
											{
										...
										protected void callService(
												AsyncCallback<ArrayList<PublicationDTO>> cb) {
											rpcArticleService.getPublicationDTOs_Normal(keys, cb);
										}
									}.retry(3);
								}
								...
								public ConceptContentsItem composeContentsItem(
										PublicationDTO dto) {
									PublicationDescItem item = new PublicationDescItem(dto);
									return item;
								}
		...
		display.getAddArticleBt().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				display.getCenterPanel().add(new addPublicationUI(sys_name,key,name));
			}
		});
