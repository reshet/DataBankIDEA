...
import com.mresearch.databank.client.views.AdminJuryPerspectiveView;
...
public class JuryAdminAppController implements ValueChangeHandler<String>, AppController {
 ...
 // private final StartPageServiceAsync startpageService;
  ...
  private static PubAdminAppControllerUiBinder uiBinder = GWT
	...
	interface PubAdminAppControllerUiBinder extends
			UiBinder<VerticalPanel, JuryAdminAppController> {
	}
  ...
  public JuryAdminAppController(SimpleEventBus eventBus) {
	//initWidget();
...
//    });
  ...
  private void doViewJurybank() {
    History.newItem("juryadmin-databank");
  }
  ...
  public void go(VerticalPanel mainPanel)
  {
	...
	if ("".equals(History.getToken())) {
      History.newItem("jurydmin-databank");
    } else {
  ...
  public void onValueChange(ValueChangeEvent<String> event) {
    ...
    if (token != null) {
      ...
      if(token.equals("juryadmin-databank")){
    	  presenter = new AdminJuryPerspectivePresenter(articleService, eventBus,new AdminJuryPerspectiveView(eventBus));
